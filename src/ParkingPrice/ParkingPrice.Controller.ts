...
export class ParkingPriceController {
  ...
  public create = async (req: Request, res: Response) => {
    try {
      ...
      return res.status(500).json({ message: 'Error al crear la tarifa', error: error.message });
    }
  };
  ...
  public findOne = async (req: Request, res: Response) => {
    try {
      ...
      if (!price) return res.status(404).json({ message: 'Tarifa no encontrada' });
      return res.status(200).json({ message: 'Tarifa encontrada', data: price });
    } catch (error: any) {
      return res.status(500).json({ message: 'Error al consultar tarifa', error: error.message });
    }
  };
  ...
  public findActive = async (req: Request, res: Response) => {
    try {
      ...
      if (!price) return res.status(404).json({ message: 'No hay tarifa activa vigente para este tipo de vehículo' });
      return res.status(200).json({ message: 'Tarifa activa encontrada', data: price });
    } catch (error: any) {
      return res.status(500).json({ message: 'Error al consultar tarifa activa', error: error.message });
    }
  };
  ...
  public remove = async (req: Request, res: Response) => {
    try {
      ...
      if (!deactivated) return res.status(404).json({ message: 'Tarifa no encontrada o ya inactiva' });
      return res.status(200).json({ message: 'Tarifa dada de baja exitosamente' });
    } catch (error: any) {
      return res.status(500).json({ message: 'Error al dar de baja la tarifa', error: error.message });
    }
  };
}